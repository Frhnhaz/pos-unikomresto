...
if (!empty($_POST['delete_order_validate'])) {
    ...
    if (mysqli_num_rows($select)) {
        $message = '<script>alert("Order telah memiliki item order, data order tidak bisa dihapus")
            window.location="../order"
            </script>;';
    } else {
        $query = mysqli_query($conn, "DELETE FROM tb_order WHERE id_order = '$kode_order'");
        if (!$query) {
            $message = '<script>alert("Pesanan gagal dihapus")
            window.location="../order"
            </script>;';
        } else {
            $message = '<script>alert("Pesanan berhasil dihapus")
            window.location="../order"
            </script>;';
        }
    }
